...
with open(join(data_root, 'example.html'), 'rU') as f:
    html = f.read()
...
print [val.text for val in b.find_all(name='li')]

# print the text of only the API resources
print [val.text for val in b.find_all(name='li') if 'API' in str(val.text)]
...
f = codecs.open(join(data_root, 'Shawshank.html'), "w", 'utf-8')
f.write(b.prettify()) 
f.close()
...
print b
# get the description
print b.find(name='p', attrs={'itemprop': 'reviewBody'}).text

# get the content rating
print b.find(name='span', attrs={'itemprop': 'contentRating'}).text

# get the duration in minutes (as an integer)
print b.find(name='time', attrs={'itemprop': 'duration'})

print b.find(name='time', attrs={'itemprop': 'duration'}).text.strip()
...
def get_movie_dict(imdb_id):
    result = {}
    try:
        html = requests.get('http://www.imdb.com/title/{}/'.format(imdb_id))
    except:
        print 'Bad input'
    b = BeautifulSoup(html.text)
    result['content_rating'] = b.find(name='span', attrs={'itemprop': 'contentRating'}).text
    result['description'] = b.find(name='p', attrs={'itemprop': 'reviewBody'}).text
    result['duration'] = b.find(name='time', attrs={'itemprop': 'duration'}).text.strip()
    result['star_rating'] = float(b.find(name='span', attrs={'itemprop': 'ratingValue'}).text)
    result['title'] = b.find(name='h1', attrs={'class': '', 'itemprop': 'name'}).text
    return result
    

# test the function
from pprint import pprint
pprint(get_movie_dict('tt0111161'))

# open the file of IDs (one ID per row), and store the IDs in a list
with open(join(data_root, 'imdb_ids.txt'), 'r') as f:
    ids = f.readlines()
    ids = [id0.strip('\n') for id0 in ids]
